...
function dataToData(schedules) {
    ...
    schedules.map((schedule) => {
        const { view, services, title: scheduleTitle, start: scheduleStart, end: scheduleEnd } = schedule;
        const scheduleObj = {
            churchId: 2,
            title: scheduleTitle,
            view,
                ...
                events.map((event, eventId) => {
                    ...
                    tasks.map((task) => {
                        if (title !== 'Cooking') {
                            ...
                            const userId = data.users.filter((el) => `${el.firstName} ${el.lastName}` === assignee)[0]
                                .id;
